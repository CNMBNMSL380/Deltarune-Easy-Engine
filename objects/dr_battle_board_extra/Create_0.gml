...
function drawBorder() {
	...
	for(var i = 0; i < ds_list_size(listVertex); i++) {
		...
		draw_sprite_ext(spr_pixel,0,x + ax - 5*cos(degtorad(floor(point_direction(ax,ay,bx,by)))),y + ay + 5*sin(degtorad(floor(point_direction(ax,ay,bx,by)))),5,point_distance(ax,ay,bx,by)+5+5*cos(degtorad((point_direction(ax,ay,bx,by)%45))),point_direction(ax,ay,bx,by)+90,dr_battle_board.color_frame,dr_battle_board.alpha_frame);
	}
}
