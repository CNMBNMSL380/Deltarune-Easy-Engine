...
function Shop_SetMenuSell(){
	...
	if(MENU = 0){
		...
		Shop_SetDialog("{choice_switch_sound false}{instant true}{define `ITEM` `"+string(Item_GetName(Item_Get(shop._sell_choice)))+"`}{define `PRICE` `"+string(Item_GetPriceSell(Item_Get(shop._sell_choice)))+"`}{font 1}{space_y 4}{scale 2}{shadow false}{gui false}{depth "+string(DEPTH_SHOP.DIALOG)+"}"+Lang_GetString("shop.menu.sell.confirm"));
