...
function Dr_Battle_SetStage(STAGE){
		...
		for(var i = 0; i < array_length(goToEvent) ; i++){
			if(goToEvent[i].friend_button == 3){
				var mercyEvent = {
					...
					friend : goToEvent[i].friend_choice_friend
				}
	...
	if(STAGE == DR_BATTLE_STAGE.BEFORE_TURN){
		...
		Dr_Battle_EnemyFunc(,DR_BATTLE_ENEMY.CREATE_TURN)
	...
	if(STAGE == DR_BATTLE_STAGE.END_TURN){
		Dr_Battle_EnemyFunc(,DR_BATTLE_ENEMY.END_TURN);
...
function Dr_Battle_ChoicePlayerMenu(BUTTON,MENU = -1,BACK = false){
	...
	switch(BUTTON){
				...
				switch(MENU){
					case DR_BATTLE_PLAYERMENU.CHOICE_ENEMY:
						if(BACK){ return Dr_Battle_SetPlayerMenu(DR_BATTLE_PLAYERMENU.BUTTON); }
							...
							Dr_Battle_SetPlayerMenu(DR_BATTLE_PLAYERMENU.CHOICE_CLASS); 
						}
					break;
			...
			switch(MENU){
				...
				case DR_BATTLE_PLAYERMENU.CHOICE_FRIEND:
					if(BACK){ Dr_Battle_SetPlayerMenu(DR_BATTLE_PLAYERMENU.CHOICE_ITEM) }
						...
						Dr_Battle_PlaySpriteAnimOnce(Main._player_friend[Main._player_friend_num],DR_PLAYER_INDEX.ITEM_IDLE);
			...
			switch(MENU){
				case DR_BATTLE_PLAYERMENU.CHOICE_ENEMY:
					if(BACK){ Dr_Battle_SetPlayerMenu(DR_BATTLE_PLAYERMENU.BUTTON) }
						...
						Dr_Battle_PlaySpriteAnimOnce(Main._player_friend[Main._player_friend_num],DR_PLAYER_INDEX.MERCY);
						Dr_Battle_NextToFriend(BUTTON,MENU,,,Main._player_choice_enemy,Main._player_friend_num);
					}
				break;
				default:
					return Dr_Battle_SetPlayerMenu(DR_BATTLE_PLAYERMENU.CHOICE_ENEMY) 
		...
		case 4://防御
			...
			//show_message(num)
			Dr_Battle_PlaySpriteAnimOnce(Main._player_friend[Main._player_friend_num],DR_PLAYER_INDEX.DEFEND);
