...
function Anim_IsExists(target, var_name) {
    for (var i = ds_list_size(global.__anim_data)-1; i >= 0; i--) {
        var anim_item = global.__anim_data[|i];
        if (var_name == undefined) {
            if (anim_item[$ "target"] == target) return true;
        } else if (anim_item[$ "target"] == target 
                 && anim_item[$ "var_name"] == var_name) {
            return true;
        }
    }
    return false;
...
function Anim_Create(target, var_name, tween_type, ease_type, start, change, duration, delay = 0) {
    // 参数规范化
    duration = max(1, abs(duration));
    delay = max(0, abs(delay));

    // 强制移除同目标旧动画
    while (Anim_IsExists(target, var_name)) {
    ...
    if (variable_global_exists(var_name)) {
        anim_struct[$ "target_type"] = 0;
        anim_struct[$ "target"] = global;
    } else if (is_struct(target)) {
        anim_struct[$ "target_type"] = 1;
    } else if (is_array(target)) {
        anim_struct[$ "target_type"] = 2;
    } else if (instance_exists(target)) {
        anim_struct[$ "target_type"] = 3;
    } else if (ds_exists(target, ds_type_list)) {
        anim_struct[$ "target_type"] = 4;
    } else if (ds_exists(target, ds_type_map)) {
        anim_struct[$ "target_type"] = 5;
    } else if (ds_exists(target, ds_type_grid)) {
        anim_struct[$ "target_type"] = 6;
    }

    // 有效性验证后添加
    if (anim_struct[$ "target_type"] != -1) {
        ds_list_add(global.__anim_data, anim_struct);
        return true;
    }
    return false;
}
...
function Anim_Destroy(target, var_name) {
    ...
    for (var i = ds_list_size(global.__anim_data)-1; i >= 0; i--) {
        var anim_item = global.__anim_data[|i];
        if ((var_name == undefined && anim_item[$ "target"] == target)
        || (anim_item[$ "target"] == target && anim_item[$ "var_name"] == var_name)) {
            delete anim_item;
            ds_list_delete(global.__anim_data, i);
            success = true;
        }
    }
    return success;
...
function Anim_Step() {
    for (var i = ds_list_size(global.__anim_data)-1; i >= 0; i--) {
        ...
        var t = anim_item[$ "time"] / anim_item[$ "duration"];
        var value = anim_item[$ "tweenstart"] + anim_item[$ "change"] * Anim_GetValue(
            anim_item[$ "tween_type"],
            anim_item[$ "ease_type"],
            t
        );
        ...
        switch (anim_item[$ "target_type"]) {
            case 0: variable_global_set(anim_item[$ "var_name"], value); break;
            case 1: variable_struct_set(anim_item[$ "target"], anim_item[$ "var_name"], value); break;
            case 2: array_set(anim_item[$ "target"], anim_item[$ "var_name"], value); break;
            case 3: variable_instance_set(anim_item[$ "target"], anim_item[$ "var_name"], value); break;
            case 4: ds_list_replace(anim_item[$ "target"], anim_item[$ "var_name"], value); break;
            case 5: ds_map_replace(anim_item[$ "target"], anim_item[$ "var_name"], value); break;
            case 6: 
                var pos = is_string(anim_item[$ "var_name"]) ? 
                    string_split(anim_item[$ "var_name"], ",") : 
                    anim_item[$ "var_name"];
                ds_grid_set(anim_item[$ "target"], pos[0], pos[1], value);
                break;
        }
    }
    return true;
