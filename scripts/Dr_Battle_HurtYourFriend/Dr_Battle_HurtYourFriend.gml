...
function Dr_Battle_HurtYourFriend(SLOT = -1, DAMG = 0 , ANIM = true){
	...
	if(SLOT != -1){
		for(var i =0; i < array_length(Main._player_friend) ; i++){
			...
			//呼叫所有队友执行受伤动作
			Dr_Battle_SetFriendSpriteMode(Main._player_friend[i],DR_PLAYER_INDEX.HURT);
			//生成伤害实例
			var damInst = instance_create_depth(Main._player_friend[i].x,Main._player_friend[i].y,0,dr_battle_menu_damageNum);
			damInst.dam_num = DAMG
			//恢复动作
			var func = function(slot){
				Dr_Battle_SetFriendSpriteMode(Main._player_friend[slot],DR_PLAYER_INDEX.HURT);
			}
			MyAlarm_Add("Hurt : " + string(i),func(i),25,true);
		}
	}
	else{
